...
/**
 * Delivery staff class that handles order pickup and delivery operations.
 * Extends Staff and manages delivery workflow.
 */
public class Delivery extends Staff {
   ...
   * @throws InvalidOrderStateException if order is not ready for delivery
   */
  public void assignOrder(Order order) {
    ...
    if (order.getStatus() != Order.Status.ReadyForDelivery) {
      throw new InvalidOrderStateException(order.getId(), order.getStatus().toString(), "assign for delivery");
    }
  ...
  public void deliverOrder(int orderId) {
    ...
    System.out.println("Delivery staff " + getName() + " delivered order #" + orderId + " to " + 
  ...
  private Order findAssignedOrder(int orderId) {
    return assignedOrders.stream()
        ...
        .findFirst()
        .orElseThrow(() -> new OrderNotFoundException("Order #" + orderId + " not assigned to delivery staff " + getName()));
  }
  ...
  public String toString() {
    return String.format("Delivery{name='%s', id='%s', assignedOrders=%d, deliveredOrders=%d}", 
